    ...
    const fetchDeposits = async () => {
      try {
        ...
        const normalizedDeposits: DepositItem[] = data.map((item: any) => ({
          ...
          // Đã sửa: dùng item.sdt thay vì item.so_dien_thoai
          sdt: item.sdt ?? 'Chưa có số điện thoại', 
          // Đã sửa: dùng item.phong thay vì item.ten_phong
          phong: item.phong ?? 'Chưa có phòng',
          // Đã sửa: dùng item.toa_nha thay vì item.ten_toa_nha
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                // Đã thêm text-black vào className bên dưới
                    ...
                    <td className="p-5">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 w-max ${
                          item.trang_thai === 'Đã thanh toán' || item.trang_thai === 'DA_THANH_TOAN' || item.trang_thai === 'Đã duyệt'
